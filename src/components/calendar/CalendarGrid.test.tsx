...
vi.mock("./DayCell", () => ({
  DayCell: ({ day, index }: { day: DayCellViewModel; index: number }) => (
    <div data-testid={`day-cell-${index}`} data-day-index={index} tabIndex={day.isToday ? 0 : -1} role="gridcell">
      {day.date.getDate()}
    </div>
  ),
}));
...
describe("CalendarGrid", () => {
  ...
  const createMockDay = (dayOffset: number, isToday = false, isCurrentMonth = true): DayCellViewModel => {
  ...
  const mockDays = Array.from(
    { length: 35 },
    (_, i) => createMockDay(i, i === 15) // 16th is "today"
  );
