...
describe('WorkoutDetailDrawer', () => {
  ...
  describe('CompletionForm - Submission', () => {
    it('should call completeWorkout with correct data', async () => {
      const user = userEvent.setup();
      mockCompleteWorkout.mockResolvedValue(undefined);
      ...
      await user.type(distanceInput, '5100');
      await user.type(durationInput, '1750');
      await user.type(hrInput, '145');
    ...
    it('should call onWorkoutCompleted after successful submission', async () => {
      const user = userEvent.setup();
      mockCompleteWorkout.mockResolvedValue(undefined);

      const onWorkoutCompleted = vi.fn();

      render(<WorkoutDetailDrawer {...defaultProps} onWorkoutCompleted={onWorkoutCompleted} />);
      ...
      await waitFor(() => {
        expect(onWorkoutCompleted).toHaveBeenCalledTimes(1);
      });
    });

    it('should show error message on submission failure', async () => {
      const user = userEvent.setup();
      mockCompleteWorkout.mockRejectedValue(new Error('Server error'));
